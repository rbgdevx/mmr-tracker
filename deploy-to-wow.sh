...
#
# What it does:
#   1. Bumps the trailing version segment in MMRTracker.toc
#      (1.4.N → 1.4.(N+1)). Done first so a malformed .toc fails
...
DEST="/Applications/World of Warcraft/_retail_/Interface/AddOns/MMRTracker"
...
# Only the trailing numeric segment is bumped (1.4.N -> 1.4.(N+1)).
# The prefix (1.4) is preserved — bump it manually for larger releases.
TOC="$SRC/MMRTracker.toc"
